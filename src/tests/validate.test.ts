...
describe('Validate', () => {
  test('validates a given object', () => {
    expect(validate({ a: (a) => a.includes('a') }, { a: 'abcde' })).toBe(true);
  });
...
describe('createObjectValidator', () => {
  test('returns a validation function', () => {
    const validator = createObjectValidator({ a: (a) => a.includes('a') });
    expect(validator({ a: 'abcde' })).toBe(true);
  });

  test('returns a validation function that fails', () => {
    const validator = createObjectValidator({ a: (a) => a.includes('z') });
